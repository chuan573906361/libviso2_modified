...
void VisualOdometryStereo::setCalibration(float f,float cu,float cv,float b)
{
  param.calib.f = f;
  param.calib.cu = cu;
  param.calib.cv = cv;
...
vector<double> VisualOdometryStereo::estimateMotion (vector<Matcher::p_match> p_matched) {
  ...
  cout<<"calib_param:cu:"<<param.calib.cu<<",cv:"<<param.calib.cv<<",f:"<<param.calib.f<<",base:"<<param.base<<endl;
...
//  cout<<"p_matched size is "<<p_matched.size()<<endl;
  for (int32_t i=0; i<(int32_t)p_matched.size(); i++)
    active.push_back(i);

//  cout<<"in get inliers active number:"<<active.size()<<endl;
...
//      cout<<"in compute observation active["<<i<<"] is"<<active[i]<<endl;
...
//      cout<<"number of "<<i<<" is "<<active[i]<<endl;
    X1p = X[active[i]];
    Y1p = Y[active[i]];
    Z1p = Z[active[i]];
