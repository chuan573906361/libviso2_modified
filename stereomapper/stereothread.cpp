...
//StereoThread::StereoThread(CalibIO *calib,QObject *parent) :
//    QThread(parent),
//    calib(calib) {
...
//      if(simg->D1[addr1] > 63)
//          cout<<"simg->D1["<<addr1<<"]:"<<simg->D1[addr1]<<endl;
      if (val<=0) {
...
StereoThread::map3d StereoThread::createCurrentMap() {
  ...
  // adjust gain
  int32_t margin = min(min(20,m.width/2),m.height/2);
  float gain_inv = 1;
  if (gain)
    gain_inv = 1.0/gain;
          ...
          if (z2>0.1 && z2<max_dist) {
            ...
            if (u2>=0 && u2<m_curr.width && v2>=0 && v2<m_curr.height) {
              ...
              if (d2>0) {
                ...
                if (fabs(x-m_curr.X[addr2])+fabs(y-m_curr.Y[addr2])+fabs(z-m_curr.Z[addr2])<0.01) {
